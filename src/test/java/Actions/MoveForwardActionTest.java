...
class MoveForwardActionTest {
    ...
    @Test
    void getName() {
        assertEquals("Move Forward", action.getName());
    }

    @Test
    void execute() {
        assertEquals(nbOfMoveForward, jumper.getPlayer().getPosition());
        action.execute(jumper);
        assertEquals(nbOfMoveForward + 1, jumper.getPlayer().getPosition());
    }
}